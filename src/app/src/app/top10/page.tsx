...
export default function Top10() {
  ...
  function validate(val: string) {
    ...
    ranking.forEach((entry, i) => {
      ...
      if (entry.type === "exact") {
        ...
        const aliasMatch = aliases.some(a => inp === norm(a));
        const wordMatch = words.some(w => inp === w);
  ...
  return (
    <div style={{ fontFamily: "'Nunito', sans-serif", background: "#FFD700", minHeight: "100vh", paddingBottom: 40, position: "relative" }}>
      ...
      {/* TÍTULO */}
      <div style={{ textAlign: "center", padding: "14px 16px 6px", fontFamily: "'Bebas Neue', sans-serif", fontSize: 22, color: "#002776", letterSpacing: 1 }}>
        {ranking[0]?.title || "—"}
      </div>
      ...
      <div style={{ padding: "0 12px 8px", display: "flex", gap: 8 }}>
        <input
          ...
          placeholder="Digite um nome ou país..."
          autoComplete="off"
          autoCorrect="off"
          spellCheck={false}
          disabled={gameOver}
        ...
        {ranking.map((entry, i) => {
          ...
          return (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, background: isCorrect ? "rgba(0,156,59,0.9)" : isRevealed ? "rgba(120,0,0,0.8)" : "rgba(0,39,118,0.88)", borderRadius: 10, padding: "9px 14px", border: `2px solid ${isCorrect ? "#FFD700" : "rgba(255,215,0,0.15)"}` }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 800, fontSize: 15, color: "white", fontStyle: isCorrect || isRevealed ? "normal" : "italic", opacity: isCorrect || isRevealed ? 1 : 0.4 }}>
      ...
      {showModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ background: "#002776", borderRadius: 16, padding: "26px 22px", maxWidth: 340, width: "100%", textAlign: "center", border: "3px solid #FFD700" }}>
            ...
            <div style={{ fontSize: 18, margin: "10px 0", letterSpacing: 2, lineHeight: 1.8 }}>
              {ranking.map((_, i) => guessed[i] ? "✅" : "❌").join("")}
            </div>
