...
export default function Top10() {
  const [allData, setAllData] = useState<RankingEntry[]>([]);
  ...
  useEffect(() => {
    fetch(CSV_URL)
      ...
      .then(text => {
        ...
        const data: RankingEntry[] = lines.map(line => {
          const cols = line.split(",");
          return {
            date: cols[0]?.trim(),
            title: cols[1]?.trim(),
            type: cols[2]?.trim() as "person" | "exact",
            position: parseInt(cols[3]),
            answer: cols[4]?.trim(),
            hint: cols[5]?.trim(),
        ...
        const idx = uniqueDates.indexOf(today);
        setDateIdx(idx >= 0 ? idx : uniqueDates.length - 1);
        setLoading(false);
      });
  }, []);
  ...
  function validate(val: string) {
    ...
    ranking.forEach((entry, i) => {
      if (guessed[i]) return;
      if (entry.type === "exact") {
        const ans = norm(entry.answer);
        const aliases = entry.aliases.map(norm);
        if (inp === ans || aliases.includes(inp)) matched.push(i);
      } else {
        const words = norm(entry.answer).split(" ").filter((w: string) => w.length > 3);
        const aliases = entry.aliases.map(norm);
        const fullMatch = norm(entry.answer) === inp;
        const aliasMatch = aliases.some((a: string) => inp === norm(a));
        const wordMatch = words.some((w: string) => inp === w);
        if (fullMatch || aliasMatch || wordMatch) matched.push(i);
      }
    });
    return matched;
  }
  ...
  return (
    <div style={{ fontFamily: "'Nunito', sans-serif", background: "#FFD700", minHeight: "100vh", paddingBottom: 40 }}>
      ...
      <div style={{ background: "#009C3B", display: "flex", alignItems: "center", justifyContent: "center", gap: 12, padding: "7px 16px" }}>
        ...
        <span style={{ color: "white", fontWeight: 700, fontSize: 13 }}>{dates[dateIdx] ? formatDate(dates[dateIdx]) : "—"}</span>
        <button onClick={() => changeDate(1)} style={{ background: "rgba(255,255,255,0.2)", border: "none", color: "white", fontSize: 18, width: 26, height: 26, borderRadius: "50%", cursor: "pointer" }}>›</button>
      </div>
      ...
      <div style={{ padding: "0 12px 8px", display: "flex", gap: 8 }}>
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && submit()} placeholder="Digite um nome ou país..." autoComplete="off" autoCorrect="off" spellCheck={false} disabled={gameOver} style={{ flex: 1, padding: "11px 14px", fontSize: 15, fontFamily: "'Nunito', sans-serif", fontWeight: 700, border: "3px solid #009C3B", borderRadius: 10, background: "white", color: "#002776", outline: "none" }} />
        <button onClick={submit} disabled={gameOver} style={{ padding: "11px 16px", background: "#009C3B", color: "white", fontWeight: 800, fontSize: 14, border: "none", borderRadius: 10, cursor: "pointer", fontFamily: "'Nunito', sans-serif" }}>OK</button>
      </div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 12px 8px", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 3 }}>
          {Array.from({ length: MAX_LIVES }).map((_, i) => (
            <div key={i} style={{ width: 20, height: 20, borderRadius: "50%", background: i < lives ? "white" : "rgba(255,255,255,0.18)", border: "1.5px solid #333", fontSize: 10, display: "flex", alignItems: "center", justifyContent: "center" }}>⚽</div>
          ))}
        </div>
        <button onClick={giveUp} disabled={gameOver} style={{ background: "transparent", border: "2px solid #cc0000", color: "#cc0000", fontWeight: 700, fontSize: 12, padding: "5px 10px", borderRadius: 8, cursor: "pointer", fontFamily: "'Nunito', sans-serif" }}>Desistir</button>
      </div>
        ...
        {ranking.map((entry, i) => {
          ...
          return (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, background: isCorrect ? "rgba(0,156,59,0.9)" : isRevealed ? "rgba(120,0,0,0.8)" : "rgba(0,39,118,0.88)", borderRadius: 10, padding: "9px 14px", border: `2px solid ${isCorrect ? "#FFD700" : "rgba(255,215,0,0.15)"}` }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 800, fontSize: 15, color: "white", fontStyle: isCorrect || isRevealed ? "normal" : "italic", opacity: isCorrect || isRevealed ? 1 : 0.4 }}>{isCorrect || isRevealed ? entry.answer : "???"}</div>
      ...
      {showModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ background: "#002776", borderRadius: 16, padding: "26px 22px", maxWidth: 340, width: "100%", textAlign: "center", border: "3px solid #FFD700" }}>
            <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 34, color: "#FFD700", letterSpacing: 2, marginBottom: 6 }}>{hits === ranking.length ? "Perfeito! 🏆" : hits >= 7 ? "Muito bem! 🌟" : "Tente amanhã! 💪"}</div>
            <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 60, color: "white", lineHeight: 1 }}>{hits}/{ranking.length}</div>
            <div style={{ fontSize: 18, margin: "10px 0", letterSpacing: 2, lineHeight: 1.8 }}>{ranking.map((_, i) => guessed[i] ? "✅" : "❌").join("")}</div>
            <p style={{ color: "rgba(255,255,255,0.75)", fontSize: 13, margin: "8px 0 18px", fontWeight: 600 }}>{hits === ranking.length ? "Você conhece demais o futebol! 🔥" : "Volte amanhã para um novo desafio."}</p>
            <button onClick={shareResult} style={{ background: "#009C3B", color: "white", border: "none", padding: "13px 24px", borderRadius: 10, fontSize: 15, fontWeight: 800, cursor: "pointer", width: "100%", marginBottom: 8, fontFamily: "'Nunito', sans-serif" }}>📤 Compartilhar resultado</button>
            <button onClick={() => setShowModal(false)} style={{ background: "transparent", border: "2px solid rgba(255,255,255,0.25)", color: "rgba(255,255,255,0.55)", padding: "9px 24px", borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: "pointer", width: "100%", fontFamily: "'Nunito', sans-serif" }}>Fechar</button>
          </div>
        </div>
      )}
      {toast.show && (
        <div style={{ position: "fixed", bottom: 20, left: "50%", transform: "translateX(-50%)", background: toast.type === "error" ? "#8B0000" : "#002776", color: toast.type === "error" ? "#FFD0D0" : "#FFD700", padding: "9px 18px", borderRadius: 8, fontWeight: 700, fontSize: 13, zIndex: 200, whiteSpace: "nowrap" }}>{toast.msg}</div>
      )}
    </div>
  );
}
